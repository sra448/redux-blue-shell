...
const collapseModelLeaves = (name, obj) => {
  if (obj[name] !== undefined) {
    ...
    const recur = map_1(x => x[name] !== undefined ? x[name] : recur(x));
    return recur(obj);
  }
};

const mapLeavesWithPath = (fn, obj, path = []) => mapObjIndexed_1((value, key) => type_1(value) === "Object" ? mapLeavesWithPath(fn, value, [].concat(_toConsumableArray(path), [key])) : fn(value, [].concat(_toConsumableArray(path), [key])), obj);
...
const createReducer = (actions, initialState) => (state = initialState, { type, args }) => {
  const path = type.split(".");
  const action = view_1(lensPath_1(path), actions);
  const modelStateLens = lensPath_1(dropLast_1(1, path));
  const modelState = view_1(modelStateLens, state);

  return action ? over_1(modelStateLens, () => action(modelState).apply(undefined, _toConsumableArray$1(args)), state) : state;
};

const init = (model, View) => {
  ...
  const actionCreators = mapLeavesWithPath((_action, path) => (...args) => {
    store.dispatch({ args, type: path.join(".") });
  }, actions);

  const effectCreators = mapLeavesWithPath((fn, path) => {
    const modelActionsLens = lensPath_1(dropLast_1(1, path));
    const actions = view_1(modelActionsLens, actionCreators);
